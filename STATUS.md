...
- **NextCut Desktop Application (Creator Kit)**: The Tauri-based desktop app (formerly "Creator Kit") has received a complete UI overhaul with elevated cards, standard design tokens, and refined typography. The application builds successfully via `pnpm tauri build` and integrates with `nextcut-sidecar` (Python FastAPI) for local AI routing.
