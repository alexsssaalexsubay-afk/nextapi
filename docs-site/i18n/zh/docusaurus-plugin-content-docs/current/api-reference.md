...
`input` 中可选字段（视模型能力而定）包括：`mode`（`fast` / `normal`）、`aspect_ratio`、`fps`（`24` / `30`）、`generate_audio`、`watermark`、`seed`、`camera_fixed`，以及 `references` 对象数组。完整定义与限制见 OpenAPI 的 `VideoInput`（如填写 `duration_seconds` 时一般需在 **4～15** 秒之间）。
