...
def test_generate_returns_job():
    route = respx.post(f"{BASE}/v1/video/generations").mock(
        return_value=httpx.Response(
            200,
            json={"id": "job_123", "status": "queued", "estimated_credits": 42},
        )
    )
    ...
    assert r == {"id": "job_123", "status": "queued", "estimated_credits": 42}


@respx.mock
def test_get_job():
    respx.get(f"{BASE}/v1/jobs/job_123").mock(
        return_value=httpx.Response(200, json={"id": "job_123", "status": "succeeded"})
    )
    with Client(api_key="sk-test") as c:
        r = c.get_job("job_123")
...
def test_error_mapping():
    respx.post(f"{BASE}/v1/video/generations").mock(
        return_value=httpx.Response(
