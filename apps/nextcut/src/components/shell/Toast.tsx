...
export function Toast() {
  ...
  return (
    <div
      className={cn(
        "fixed bottom-8 left-1/2 z-50 -translate-x-1/2 rounded-[var(--radius-md)] border border-nc-error/20 bg-nc-surface px-5 py-3 shadow-2xl",
        "flex items-center gap-3 transition-all duration-300",
        visible ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0"
      )}
    >
      <span className="h-[6px] w-[6px] shrink-0 rounded-full bg-nc-error" />
      <span className="text-[12px] text-nc-text-secondary">{message}</span>
      <button
        onClick={() => {
          setVisible(false);
          setTimeout(() => setLastError(null), 300);
        }}
        className="ml-2 text-nc-text-ghost hover:text-nc-text-tertiary"
      >
