...
func main() {
	...
	directorSvc.SetStoryPlanner(vimaxruntime.NewRunner(vimaxruntime.RunnerConfig{
		...
		AllowFallback:   directorRuntimeAllowFallback(),
	}))
	...
	directorGroup.GET("/runs", directorH.ListRuns)
	...
	internal.POST("/ai-providers/:id/quota/sync", ah.SyncAIProviderQuota)
	internal.POST("/ai-providers/:id/quota/manual", ah.RecordAIProviderQuota)
	internal.GET("/ai-provider-logs", ah.ListAIProviderLogs)
	internal.GET("/provider-quotas", ah.ListProviderQuotas)
...
func directorRuntimeAllowFallback() bool {
	if envBool("VIMAX_RUNTIME_DISABLE_FALLBACK") {
		return false
	}
	return envBool("VIMAX_RUNTIME_ALLOW_FALLBACK")
}
