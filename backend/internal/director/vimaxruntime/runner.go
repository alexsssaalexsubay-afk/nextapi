...
func normalizeStoryboard(out *director.Storyboard, in director.GenerateShotsInput) (*director.Storyboard, error) {
	...
	for i := range out.Shots {
		...
		director.EnrichShotForGeneration(&out.Shots[i], in)
	}
	return out, nil
}
...
4. 每个镜头都必须像导演交给执行团队的 shot brief：包含镜头运动、主体锁定、参考素材策略、质量词、音频意图。
5. 不要提及任何第三方开源项目或上游供应商名称。
6. 不要包含 API key、base_url 或任何外部调用配置。
